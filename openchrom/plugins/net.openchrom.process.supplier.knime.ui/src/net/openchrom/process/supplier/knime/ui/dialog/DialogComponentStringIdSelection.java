 ...
 * -------------------------------------------------------------------
 * History
 * Most of code has been copied from class DialogComponentString
 * author: Jan Holy
 */
 ...
 *
 */
public final class DialogComponentStringIdSelection extends DialogComponent {
	 ...
	 * @param list
	 *            list (not empty) of strings (not null) for the combobox
	 * @param editable
	 *            true if the user should be able to add a value to the
	 *            combo box
	 * @param fvm
	 *            model exposed to choose from available flow variables
	 *
	...
	public DialogComponentStringIdSelection(final SettingsModelString stringModel, final String label, final Collection<String> list, Map<String, String> ids, final boolean editable) {
		...
		if(editable) {
			final StringIconListCellEditor editor = new StringIconListCellEditor();
	 ...
	 *            label for dialog in front of combobox
	 * @param list
	 *            list (not empty) of {@link StringIconOption}s for
	...
	public DialogComponentStringIdSelection(final SettingsModelString stringModel, final String label, final StringIconOption[] list, Map<String, String> ids) {
		...
		if(ids == null) {
			throw new IllegalArgumentException("Map ids list " + "shouldn't be null");
		}
		Arrays.stream(list).map(StringIconOption::getText).forEach(name -> {
			if(!ids.containsKey(name)) {
				throw new IllegalArgumentException("Map ids does not conteins for each member of Selelist");
			}
		});
	...
	private void updateModel() throws InvalidSettingsException {
		...
		String selection = ((StringIconOption)m_combobox.getSelectedItem()).getText();
		((SettingsModelString)getModel()).setStringValue(ids.get(selection));
	}
