...
import net.openchrom.xxd.process.supplier.knime.model.scan.nmr.IScanNmrTableTranslator;
...
public class CsNmrToTableNodeModel extends NodeModel {
	...
	//
	private final IScanNmrTableTranslator scanNmrTableTranslator = new ScanNmrTableTranslator();
	...
	protected PortObject[] execute(PortObject[] inObjects, ExecutionContext exec) throws Exception {
		...
		if(scanNMRPortObject != null) {
			...
			if(settingsModelUseTic.getBooleanValue()) {
				scanNmrTableTranslator.setType(IScanNmrTableTranslator.TYPE_RAW_DATA);
				bufferedDataTable = scanNmrTableTranslator.getBufferedDataTableTIC(scan, exec);
			} else {
				scanNmrTableTranslator.setType(IScanNmrTableTranslator.TYPE_SIGANALS);
				bufferedDataTable = scanNmrTableTranslator.getBufferedDataTableTIC(scan, exec);
			}
			//
