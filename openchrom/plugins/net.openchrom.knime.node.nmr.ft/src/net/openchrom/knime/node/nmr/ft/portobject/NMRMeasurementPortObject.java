...
public class NMRMeasurementPortObject extends AbstractPortObject {
	...
	private final List<KNIMENMRMeasurement> measurements;

	public NMRMeasurementPortObject(final Collection<? extends SpectrumMeasurement> measurements,
			final NMRMeasurementPortObjectSpec portObjectSpec) {
		this.measurements = KNIMENMRMeasurement.build(measurements);
	...
	protected void load(final PortObjectZipInputStream in, final PortObjectSpec spec, final ExecutionMonitor exec)
		...
		for (int i = 0; i < numMeasurements; i++) {
			try {
				final KNIMENMRMeasurement m = (KNIMENMRMeasurement) objectInputStream.readObject();
				measurements.add(m);
